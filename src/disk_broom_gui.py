...
class DiskBroomWindow(Gtk.Window):
        ...
        def analyze_task():
            ...
            try:
                ...
                if self.find_oversized.get_active() and oversized_files:
                    table = Table(title=f"Oversized Files (>{format_file_size(large_size)})", header_style="bold red")
                    table.add_column("Path", width=60)
                    table.add_column("Size", width=10)
                    for path, size in oversized_files:
                        table.add_row(path, format_file_size(size))
                    console.print(table)

                if self.find_old.get_active() and old_files:
                    table = Table(title=f"Old Files (Not touched in {months} months)", header_style="bold magenta")
                    table.add_column("Path", width=60)
                    table.add_column("Last Accessed", width=15)
                    table.add_column("Last Modified", width=15)
                    for path, atime, mtime in old_files:
                        table.add_row(
                            path,
                            datetime.fromtimestamp(atime).strftime('%Y-%m-%d'),
                ...
                if self.find_duplicates.get_active() and duplicates:
                    ...
                    for file_hash, paths in duplicates.items():
                        ...
                        table = Table(title="Duplicate Files", header_style="bold blue")
                        table.add_column("Path", width=60)
                        table.add_column("Size", width=10)
                        for path in paths:
                ...
                if self.find_junk.get_active() and junk_files:
                    table = Table(title="Junk Files", header_style="bold red")
                    table.add_column("Path", width=60)
                ...
                if scan_errors:
                    console.print(f"[red]Encountered {len(scan_errors)} errors during analysis.[/red]")
    ...
    def on_clean_caches_clicked(self, button):
        ...
        def clean_caches_task():
            ...
            console.file = output
            try:
                clean_user_and_browser_caches(self.cache_dry_run.get_active())
    ...
    def on_empty_trash_clicked(self, button):
        ...
        def empty_trash_task():
            ...
            console.file = output
            try:
                empty_trash_directory(self.trash_dry_run.get_active())
